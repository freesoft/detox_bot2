...
We've seen difficulty especially when trying to move our classifier from scikit-learn to pyspark dataframe ml libraries. First most of the pypsark ml libraries are still RDD based instead of dataframe based, second most of the spark enviornment is still mainly tested in linux enviornment. In order to make it work in the Windows enviornment there are a lot of special setups that needs to happen. We've seen that the F1 score using the pyspark dataframe ml library is slightly lower than the F1 score using scikit-learn library. In the up coming weeks we will work on improving the pyspark F1 score. 
